//! Beacon entry point: init state from env, spawn the background prober, serve.
//!
//! Also exposes a dependency-free `beacon healthcheck` subcommand used as the container
...
async fn main() {
    ...
    let state = match beacon::build_state_from_env().await {
        Ok(state) => state,
        Err(e) => {
    ...
    // Bind (and start listening) BEFORE the first probe sweep so the server is reachable
    // and self-targeted checks don't see a startup race.
...
fn run_healthcheck() -> i32 {
    let bind_addr = std::env::var("BIND_ADDR").unwrap_or_else(|_| "127.0.0.1:8400".to_string());
    // Healthcheck always talks to the loopback regardless of the bind interface.
    let port = bind_addr.rsplit(':').next().unwrap_or("8400");
